...
def validate_utilities(assets):
    ...
    # Outfit availability is owned by the independently edited ArmoryCatalog.
    # This authorer must not add the utilities to ambient traffic/parking.
    ambient=re.search(r'string\[\] Models\s*=\s*\{(.*?)\}',catalog,re.S)[1]
    for car in cars:
        assert car['id'] not in ambient and car['name'] not in ambient
        path=ua.ROOT/f'{ua.ASSET}/Prefabs/{car["id"]}.prefab';docs=documents(path)
        renderers=[d['MeshRenderer'] for t,d in docs.values() if t==23]
        assert len(renderers)==7 and len({r['m_Materials'][0]['guid'] for r in renderers})==3
    ...
    print('PASS: 6 showroom utilities, closed tires, open glazed apertures, cabin ceilings and shared materials; absent from ambient passenger pool.')
    return cars
