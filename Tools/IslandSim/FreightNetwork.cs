...
sealed class FreightNetwork
{
    ...
    public FreightNetwork(JsonElement fixture,LaneNet net)
    {
        foreach(var district in fixture.GetProperty("districts").EnumerateArray())
        {
            ...
            {
                var estate=new IndustrialDistrict {compact=true,pocket=true,Frame=frame}; estate.Plan(null,seed);
            ...
            else if(kind==DistrictKind.Harbor)
            {
                ...
                var graph=HarborDemo.HarborStreet.Build(frame,new[]{-harbor.QuayHalf-10f,0f,240f,harbor.QuayHalf+250f},-7.5f);
                RegionalRoads.Join(net,graph.Edges);
                _portals.Add((frame.ToWorld(Vector3.zero),graph.Nodes[1]));
                _portals.Add((frame.ToWorld(new Vector3(240,0,0)),graph.Nodes[2]));
            }
        }
    }
