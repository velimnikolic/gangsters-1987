...
namespace LivingCity.UI
{
    ...
    public sealed partial class PersonnelAlmanac
    {
        ...
        void BuildTable(IReadOnlyList<Gangs.Gang> gangs)
        {
            ...
            // ---- what goes with the word ----
            y = TableBand(sheet, y, "WHAT GOES WITH IT");
            var safe = outfit.Accounts.Safe;
            tableMoney = Mathf.Clamp(tableMoney, 0, Mathf.Max(0, safe));
            LedgerV2.Mono(sheet, 0f, y, 80f, "MONEY", 9.5f, LedgerV2.Label, 6f);
            LedgerV2.Button(sheet, "−", 90f, y + 2f, 30f, TableKeyH, () =>
            {
            ...
            LedgerV2.Button(sheet, "+", 262f, y + 2f, 30f, TableKeyH, () =>
            {
                tableMoney = Mathf.Min(Mathf.Max(0, safe), tableMoney + TableMoneyStep);
                dirty = true;
            }, red: false, size: 11f, outline: true);
            Line(sheet, LedgerStyle.MonoItalic, 11f, LedgerV2.Muted, 300f, y, w - 300f,
                LineBox(11f), "rides on a truce, a peace, a bill, the tribute's terms or a " +
                              "joined war · " + LedgerText.Cash(safe) + " in the safe");
            y -= TableKeyH + 4f;

            CollectStreets();
            tableStreet = tableStreets.Count > 0 ? Mathf.Clamp(tableStreet, 0, tableStreets.Count - 1) : 0;
            LedgerV2.Mono(sheet, 0f, y, 80f, "STREET", 9.5f, LedgerV2.Label, 6f);
            LedgerV2.Button(sheet, "◂", 90f, y + 2f, 30f, TableKeyH, () =>
            {
                if (tableStreets.Count > 0)
                    tableStreet = (tableStreet + tableStreets.Count - 1) % tableStreets.Count;
                dirty = true;
            }, red: false, size: 11f, outline: true);
            var street = Line(sheet, LedgerStyle.MonoBold, 12f, LedgerV2.Ink, 126f, y, 300f,
                LineBox(12f), tableStreets.Count > 0
                    ? StreetName(tableStreets[tableStreet])
                    : "no street of ours to name");
            street.overflowMode = TextOverflowModes.Ellipsis;
            LedgerV2.Button(sheet, "▸", 432f, y + 2f, 30f, TableKeyH, () =>
            {
                if (tableStreets.Count > 0)
                    tableStreet = (tableStreet + 1) % tableStreets.Count;
                dirty = true;
            }, red: false, size: 11f, outline: true);
            Line(sheet, LedgerStyle.MonoItalic, 11f, LedgerV2.Muted, 470f, y, w - 470f,
                LineBox(11f), "a warning or a threat keeps them off it · a line keeps us both off it");
            y -= TableKeyH + 4f;

            CollectThirds(gangs, house.Id);
            tableThird = tableThirds.Count > 0 ? Mathf.Clamp(tableThird, 0, tableThirds.Count - 1) : 0;
            LedgerV2.Mono(sheet, 0f, y, 80f, "AGAINST", 9.5f, LedgerV2.Label, 6f);
            LedgerV2.Button(sheet, "◂", 90f, y + 2f, 30f, TableKeyH, () =>
            {
                if (tableThirds.Count > 0)
                    tableThird = (tableThird + tableThirds.Count - 1) % tableThirds.Count;
                dirty = true;
            }, red: false, size: 11f, outline: true);
            var third = Line(sheet, LedgerStyle.MonoBold, 12f, LedgerV2.Ink, 126f, y, 300f,
                LineBox(12f), tableThirds.Count > 0 ? tableThirds[tableThird].Name : "no third house");
            third.overflowMode = TextOverflowModes.Ellipsis;
            LedgerV2.Button(sheet, "▸", 432f, y + 2f, 30f, TableKeyH, () =>
            {
                if (tableThirds.Count > 0)
                    tableThird = (tableThird + 1) % tableThirds.Count;
                dirty = true;
            }, red: false, size: 11f, outline: true);
            Line(sheet, LedgerStyle.MonoItalic, 11f, LedgerV2.Muted, 470f, y, w - 470f,
                LineBox(11f), "the house a pact is sworn against, or the war they are asked into");
            y -= TableKeyH + 4f;
            ...
            LedgerV2.Chip(sheet, "IN PERSON", 226f, y + 2f, 110f, TableKeyH, tableInPerson, () =>
            {
            ...
            {
                LedgerV2.Button(sheet, "◂", 344f, y + 2f, 30f, TableKeyH, () =>
                {
                ...
                LedgerV2.Button(sheet, "▸", 646f, y + 2f, 30f, TableKeyH, () =>
                {
            ...
            var keyW = (w - 5f * 6f) / 6f;
            var x = 0f;
            LedgerV2.Button(sheet, "DECLARE WAR", x, y, keyW, TableKeyH, () => Stance(house.Id, Outfit.Stance.War),
                red: true, size: 10f, outline: current == Outfit.Stance.War);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "OFFER TRUCE", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.OfferTruce),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "OFFER PEACE", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.OfferPeace),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "WARNING", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.Warn),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "THREAT", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.Threaten),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "SEND A BILL", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.Bill),
                red: false, size: 10f, outline: true);
            y -= TableKeyH + 6f;

            x = 0f;
            LedgerV2.Button(sheet, "TRIBUTE TERMS", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.TributeTerms),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "DRAW A LINE", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.Line),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "OFFER A PACT", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.Pact),
                red: false, size: 10f, outline: true);
            x += keyW + 6f;
            LedgerV2.Button(sheet, "JOIN MY WAR", x, y, keyW, TableKeyH, () => Say(house.Id, ProposalKind.JoinWar),
                red: false, size: 10f, outline: true);
            y -= TableKeyH + 4f;
            Paragraph(sheet, LedgerStyle.Mono, 11f, LedgerV2.Muted, 0f, y, w, 40f,
                "War is declared and lands at midnight. Everything else is asked, and " +
                "their desk answers at once - or, carried in person, when our man " +
                "stands at their door. A beaten house cannot refuse a truce; a word " +
                "refused or left two days is owed for; a bill is priced from what " +
                "they owe us; a line is taken only by two houses that could not pay " +
                "for a war.", lineSpacing: 2f);
            y -= 46f;
        ...
        void CollectThirds(IReadOnlyList<Gangs.Gang> gangs, int except)
        {
            tableThirds.Clear();
            foreach (var gang in gangs)
