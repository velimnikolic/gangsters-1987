...
namespace LivingCity.Tests
{
    ...
    public static class LoyaltyTests
    {
        ...
        static void EveryHouseWalksOutOfItsOwnCity(List<string> failures)
        {
            ...
            var runner = new CampaignRunner { GangId = 7 };
            ...
            if (runner.Relations.TryGetPending(1, out _))
        ...
        static void SouringNeverSoftensAnOrderHeGave(List<string> failures)
        {
            ...
            var runner = new CampaignRunner { GangId = Gangs.GangCatalog.PlayerGangId };
            runner.Campaign.Day = 5;
            ...
            runner.Relations.SetPending(3, Stance.War);
            runner.DayTick(roster, payTribute: false);
            ...
            if (runner.Defections.Count == 0 ||
                ...
                failures.Add("SouringNeverSoftensAnOrderHeGave: the fixture's man did " +
                             "not walk to house 3.");
            if (runner.Relations.StanceWith(3) != Stance.War)
                failures.Add("SouringNeverSoftensAnOrderHeGave: his declared war came " +
                             "out of the midnight as " + runner.Relations.StanceWith(3) +
                             ".");
            ...
            third.Loyalty = 0;
            var softening = new CampaignRunner { GangId = Gangs.GangCatalog.PlayerGangId };
            softening.Campaign.Day = 5;
            softening.HoldingsOf = into => { into.Clear(); into.AddRange(city); };
            softening.Relations.SetPending(3, Stance.Truce);
            softening.Relations.ApplyPending();
            softening.Relations.SetPending(3, Stance.Peace);
            softening.DayTick(soft, payTribute: false);
            if (softening.Relations.StanceWith(3) != Stance.War)
                failures.Add("SouringNeverSoftensAnOrderHeGave: a house that took our " +
                             "men was let down to " +
                             softening.Relations.StanceWith(3) + " by an order he gave " +
                             "before it happened.");
            ...
            var second = AddCrew(quiet, "Second", AttributeScale.MaxHalfSteps, 4, 90,
                out _);
            second.Loyalty = 0;
            var other = new CampaignRunner { GangId = Gangs.GangCatalog.PlayerGangId };
            other.Campaign.Day = 5;
            other.HoldingsOf = into => { into.Clear(); into.AddRange(city); };
            other.DayTick(quiet, payTribute: false);
            if (other.Relations.StanceWith(3) != Stance.Truce)
                failures.Add("SouringNeverSoftensAnOrderHeGave: a house that took our " +
                             "men stayed at " + other.Relations.StanceWith(3) + ".");
        }
