...
namespace LivingCity.EditorTools
{
    ...
    {
        public const string TablePath = "Assets/Resources/ResidentialDecor.json";
        ...
        static void WriteTable(List<Rule> rules, int[] styles, float escapeMin,
                               ...
                               List<string> unresolved)
        {
            // Match the old C# literal precision exactly (not raw binary rounding).
            float F(float value) => float.Parse(ResidentialForgeSource.Float(value).TrimEnd('f'),
                System.Globalization.CultureInfo.InvariantCulture);
            var catalog = new RoadDemo.ResidentialDecorCatalog
            {
                Version = 1, StyleWeights = styles,
                FireEscapeShareMin = F(escapeMin), FireEscapeShareMax = F(escapeMax),
                NotReady = notReadyFamilies,
                Unresolved = unresolved.Distinct().OrderBy(s => s, StringComparer.Ordinal).ToArray(),
                All = rules.Select(r => new RoadDemo.ResidentialDecorRule
                {
                    Family = r.Family, Prefab = r.Prefab, Variant = r.Variant,
                    Anchor = (RoadDemo.ResidentialDecorAnchor)Enum.Parse(typeof(RoadDemo.ResidentialDecorAnchor), r.Anchor.ToString()),
                    Relation = (RoadDemo.ResidentialDecorRelation)Enum.Parse(typeof(RoadDemo.ResidentialDecorRelation), r.Relation.ToString()),
                    HostKind = (RoadDemo.ResidentialModuleKind)Enum.Parse(typeof(RoadDemo.ResidentialModuleKind), r.HostKind.ToString()),
                    X = F(r.X), Y = F(r.Y), Z = F(r.Z), Yaw = F(r.Yaw),
                    Min = F(r.Min), Mean = F(r.Mean), Max = F(r.Max),
                    FamilyMin = F(r.FamilyMin), FamilyMean = F(r.FamilyMean), FamilyMax = F(r.FamilyMax),
                    HostStyle = r.HostStyle, Count = r.Count, Buildings = r.Buildings,
                    Part = r.Part, Parts = r.Parts, Span = r.Span,
                    ColumnOffset = r.ColumnOffset, RowOffset = r.RowOffset, FloorOffset = r.FloorOffset,
                    Role = r.Role, VariantWeight = r.VariantWeight,
                    UnboundShare = F(r.UnboundShare), Repeat = r.Repeat, Ready = r.Ready,
                }).ToArray(),
            };
            catalog.Validate();
            // One rule per line, so generated diffs remain reviewable.
            string json = JsonUtility.ToJson(catalog).Replace("},{", "},\n{");
            ...
            RoadDemo.ResidentialDecor.Invalidate();
        }
