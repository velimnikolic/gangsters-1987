...
namespace RoadDemo
{
    ...
    public readonly struct MapSheet
    {
        ...
        /// <summary>Metres of ground to one AUTHORED unit. Real pixels are three to
        /// the unit, so a metre figure for the player is <see cref="RealMetres"/>.</summary>
        ...
        /// <summary>Authored units to the metre.</summary>
        public float PixelsPerMetre => 1f / Metres;

        /// <summary>Real pixels to the metre - what the detail passes draw in.</summary>
        public float RealPerMetre => MapRaster.S / Metres;

        /// <summary>Metres to one REAL pixel. THE scale, and the only one the player is
        /// ever shown: the footer prints it and the inspector's footprints agree with
        /// it. Two scales in one interface is how a map starts lying.</summary>
        public float RealMetres => Metres / MapRaster.S;

        /// <summary>The ground the sheet covers.</summary>
        public Rect Window => new Rect(
            Centre.x - MapRaster.AW * 0.5f * Metres,
            Centre.y - MapRaster.AH * 0.5f * Metres,
            MapRaster.AW * Metres, MapRaster.AH * Metres);

        /// <summary>The ground it covers with a margin, for culling things whose ink
        /// spills past their own footprint.</summary>
        public Rect Margin(float authoredPixels)
        {
            var window = Window;
            var pad = authoredPixels * Metres;
        ...
        public Vector2 ToPx(Vector2 world) => new Vector2(
            MapRaster.AW * 0.5f + (world.x - Centre.x) / Metres,
            MapRaster.AH * 0.5f - (world.y - Centre.y) / Metres);

        public Vector2 ToPx(Vector3 world) => ToPx(new Vector2(world.x, world.z));

        /// <summary>The same point in REAL pixels - what anything drawn at full
        /// resolution wants: a sprite, a window, a kerb, a lane marking.</summary>
        public Vector2 ToReal(Vector2 world) => new Vector2(
            MapRaster.W * 0.5f + (world.x - Centre.x) * MapRaster.S / Metres,
            MapRaster.H * 0.5f - (world.y - Centre.y) * MapRaster.S / Metres);

        public Vector2 ToReal(Vector3 world) => ToReal(new Vector2(world.x, world.z));

        /// <summary>And back: the ground under a raster pixel.</summary>
        /// <summary>The ground under an AUTHORED point - the space every pick works
        /// in.</summary>
        public Vector2 ToWorld(Vector2 px) => new Vector2(
            Centre.x + (px.x - MapRaster.AW * 0.5f) * Metres,
            Centre.y - (px.y - MapRaster.AH * 0.5f) * Metres);
        ...
        {
            var x0 = Mathf.RoundToInt(MapRaster.AW * 0.5f + (world.xMin - Centre.x) / Metres);
            var x1 = Mathf.RoundToInt(MapRaster.AW * 0.5f + (world.xMax - Centre.x) / Metres);
            // yMax is NORTH, which is the top of the sheet and therefore the low row.
            var y0 = Mathf.RoundToInt(MapRaster.AH * 0.5f - (world.yMax - Centre.y) / Metres);
            var y1 = Mathf.RoundToInt(MapRaster.AH * 0.5f - (world.yMin - Centre.y) / Metres);
            return new RectInt(x0, y0, Mathf.Max(1, x1 - x0), Mathf.Max(1, y1 - y0));
        }

        /// <summary>
        /// A world rectangle in REAL pixels. Computed from the world and not by
        /// multiplying the authored box, so a footprint keeps its true proportions
        /// instead of being quantised to the authored grid and then trebled - the
        /// difference between a row of sheds that vary and a row that steps in threes.
        /// </summary>
        public RectInt RealBox(Rect world)
        {
            var scale = MapRaster.S / Metres;
            var x0 = Mathf.RoundToInt(MapRaster.W * 0.5f + (world.xMin - Centre.x) * scale);
            var x1 = Mathf.RoundToInt(MapRaster.W * 0.5f + (world.xMax - Centre.x) * scale);
            var y0 = Mathf.RoundToInt(MapRaster.H * 0.5f - (world.yMax - Centre.y) * scale);
            var y1 = Mathf.RoundToInt(MapRaster.H * 0.5f - (world.yMin - Centre.y) * scale);
            return new RectInt(x0, y0, Mathf.Max(1, x1 - x0), Mathf.Max(1, y1 - y0));
        }
