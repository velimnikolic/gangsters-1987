...
* **Nothing until the shooting starts (2026-09-04, the user's word: "treba da traze zaklon kad krene pucacina").** A crew sent to a fight asks for no cover, and every man of it closes, until its fight is HOT - a round fired by it or at it inside `FightHotFor` 8 s (`Unit.ShotAt` / `ProvokedAt`). Then the parts above apply.
