...
| The deed | `Scripts/Personnel/Sentencing.cs` — `Deed {Affray, Murder, CopKilling, Extortion, WitnessTampering…}` (appended values keep serialized meaning), `Days(...)`, `BandLow/BandHigh`, `ChargeFor`, `Bail`, `EscapeSurcharge` 4, `DaysToCourt` 5, `ExtraCountDays` | two new deeds |
